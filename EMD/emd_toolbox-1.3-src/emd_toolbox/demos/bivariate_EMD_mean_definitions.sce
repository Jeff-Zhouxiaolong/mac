...
clc;  f=scf(); fig_id=get(gcf(),"figure_id"); clf; mode(1);lines(0);
...
function y=signal(t)
y = emd_dirstretch(exp(%i*t),%pi/3,2);
endfunction
...
A = 2.5*[-1,1,-1,1];
label_pos = [.05,.05];
	  xstring(A(1)+(A(2)-A(1))*label_pos(1),A(3)+(A(4)-A(3))*(1-label_pos(2)),['('+label+')']);
endfunction
small_bar = .25;
big_bar = .7;
...
function plotbar(x,dirs,sb,bb)
   tmp=[x-bb*dirs,x-sb*dirs*%i;x+bb*dirs,x+sb*dirs*%i];
 plot(real(tmp),imag(tmp),'k');//,'Color','k','LineWidth',L);
endfunction
if ~show_errorbars
  function plotbar(x,dirs,sb,bb)
    [];
...
// draws bigpoints
function plotpoint(x,col)
M = 5;
  plot(real(x),imag(x),[col+'o']);//,'MarkerSize',M,'MarkerFaceColor',col);
endfunction
// draws a line in direction dir passing through point x
function tline(x,dir,varargin)
  plot(real([x-Length*exp(i*dir);x+Length*exp(i*dir)]),imag([x-Length*exp(i*dir);x+Length*exp(i*dir)]),varargin(:));
endfunction
figtitle = 'Definition of the mean of the envelope';

f.figure_name=figtitle;
// first algorithm
subplot(121)
...
plotpoint(x(inds),'b')
if show_polygon
  plot(real(x([inds,inds(1)])),imag(x([inds,inds(1)])),'b')
end
...
if pmodulo(Ndirs,2)==0 & show_midtangents
  inds2 = 1:Ndirs/2;
