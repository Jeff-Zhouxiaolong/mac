...
int c2_stop_sifting(COMPLEX_T *, double *,extrema_t *,stop_t *,int,int);
 ...
 int int_cemdc2(char *fname)
{
  ...
  while ((!max_imfs || (nb_imfs < max_imfs)) && !stop_EMD) {
    ...
    while (!stop_status && !c2_stop_sifting(m,a,&ex,&stop_params,n,iteration_counter)) {
...
int c2_stop_sifting(COMPLEX_T *m, double *a,extrema_t *ex,stop_t *sp,int n, int counter) {
  int i,count;
  double tol,eps;
  tol = sp->tolerance*n;
  eps = sp->threshold;
