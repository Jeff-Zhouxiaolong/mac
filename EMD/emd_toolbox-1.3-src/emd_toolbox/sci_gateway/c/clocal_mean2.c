...
envelope_t c2_init_local_mean(int n) {
  envelope_t env;
...
void c2_free_local_mean(envelope_t env) {
  free(env.e_min);
  free(env.e_max);
  free(env.tmp1);
  free(env.tmp2);
...
int c2_mean_and_amplitude(double *x,COMPLEX_T *z,COMPLEX_T *m,double *a,int n,int nbphases,extrema_t *ex,envelope_t *env) {
  int i,k;
  #ifdef C99_OK
  COMPLEX_T eiphi;
  #else
...
int c2_mean(double *x,COMPLEX_T *z,COMPLEX_T *m,int n,int nbphases,extrema_t *ex,envelope_t *env) {
  int i,k;
  #ifdef C99_OK
  COMPLEX_T eiphi;
  #else
