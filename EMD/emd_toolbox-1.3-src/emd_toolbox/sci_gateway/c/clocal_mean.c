...
envelope_t c_init_local_mean(int n) {
  envelope_t env;
...
void c_free_local_mean(envelope_t env) {
  free(env.re_min);
  free(env.ie_min);
  free(env.re_max);
  free(env.ie_max);
...
int c_mean_and_amplitude(double *x,COMPLEX_T *z,COMPLEX_T *m,double *a,int n,int nbphases,extrema_t *ex,envelope_t *env) {
  int i,k;
  double phi;
  ...
  for(k=0;k<nbphases;k++) {
    ...
    c_boundary_conditions(x,z,phi,n,ex);
    ...
    if ((ex->n_min > LIM_GMP)||(ex->n_min > LIM_GMP)) {
      sciprint("Too many extrema, interpolation may be unreliable\n");
    }
...
int c_mean(double *x,COMPLEX_T *z,COMPLEX_T *m,int n,int nbphases,extrema_t *ex,envelope_t *env) {
  int i,k;
  double phi;
